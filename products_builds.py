...
async def fetch_product_ids(session, page_id):
    url = f"https://catalog.gog.com/v1/catalog?limit=1000&order=asc&page={page_id}&countryCode=AU&locale=en-US&currencyCode=AUD"
    async with session.get(url) as response:
        data = await response.json()
        return data.get('pages', 0), [product['id'] for product in data.get('products', [])]
...
async def fetch_product(session, product_id, operating_system, generation):
    ...
    async with session.get(url) as response:
        if response.content_type == 'application/json':
            ...
            print(f"Unexpected content type for {url}: {response.content_type}")
            return None
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for page_id in range(start_page_id, end_page_id + 1):
            _, product_ids = await fetch_product_ids(session, page_id)
            tasks = [download_and_save(session, product_id, os_type, generation, semaphore, hashes, completed_tasks) for product_id in product_ids]
    ...
    for original_key, value in sorted(hashes.items(), key=lambda item: int(item[0]) if str(item[0]).isdigit() else float('inf')):
        if str(original_key).isdigit():
            sorted_key = f"products/{original_key}/os/{os_type}/builds@generation={generation}"
